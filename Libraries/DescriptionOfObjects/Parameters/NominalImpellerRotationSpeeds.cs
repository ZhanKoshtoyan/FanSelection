namespace Libraries.DescriptionOfObjects.Parameters;

public static class NominalImpellerRotationSpeeds
{
    /// <summary>
    ///     Скорость вращения крыльчатки: короткое название
    /// </summary>
    public static readonly double[] Values = { 3000, 1500, 1000, 750 };

    /// <summary>
    ///     Скорость вращения крыльчатки: полное название
    /// </summary>
    public static readonly string[] Names = { "3000", "1500", "1000", "750" };

    public static readonly double[] NumberOfPoles = { 2, 4, 6, 8 };
}
